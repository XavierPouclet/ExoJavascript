...
function get_page(page_url){
	...
	request({ url:page_url,}, function(error, http_client_response, html_str){
		...
		var page_len = http_client_response.headers.['Content-length'];
		var page_serv = http_client_response.headers.['server'];
		var page_type = http_client_response.headers.['Content-type'];

		em.emit('page', page_url, html_str, page_len, page_serv, page_type);

		var urlTemp = queue.shift();
		//stock_url.push(urlTemp);
		get_page(urlTemp);
	});
}
...
em.on('page:scraping', function(page_url){ console.log('Loading... ', page_url); });
 
	// Listen to events, see: http://nodejs.org/api/all.html#all_emitter_on_event_listener
em.on('page', function(page_url, html_str, page_len, page_serv, page_type){ 
	console.log('We got a new page!', page_url, page_len, page_serv, page_type); 
});
 
em.on('page', extract_links);

em.on('page:error', function(page_url, error){ console.error('Oops an error occured on', page_url, ' : ', error); });
 
em.on('url', function(page_url, html_str, url){ console.log('We got a link! ', url); });
...
app.get('/queue/size', function(req, res){ res.setHeader('Content-Type', 'text/plain'); res.json(200, {queue:{length:queue.length}}); });
 
app.get('/queue/add', function(req, res){
	var url = req.param('url');
	get_page(url);
	res.json(200, {
		queue:{ added:url, length:queue.length }
	});
});
 
app.get('/queue/list', function(req, res){
	res.json(200, {
		queue:{ length:queue.length, urls:queue }
	});
});
